...
For each question:
#### Q: [question text]
**Answer:** (Detailed finding explaining the logic and implications)
**Evidence:** (file:line references)

### Key Insights
(The most important things for a reviewer, specific to your concern area. Be highly detailed and reference exact functions/lines.)

### Risks Found
(Anything risky or inconsistent — be highly specific with exact file:line references)
...
async function runSingleAnalyst(config: {
  ...
  if (largeFilesSummaries) {
    userParts.push(
      `\n## Large Files Summaries\n` +
      `The following large files have been summarized to save context window space. You may use get_diff_for_file to see their full content if necessary:\n\n` +
      largeFilesSummaries
    );
  }
