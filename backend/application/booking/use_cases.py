...
class CreateBookingUseCase:
    ...
    async def execute(self, tenant_id: UUID, data: BookingCreate) -> Booking:
        ...
        # Check Availability
        conflicts = await self.booking_repo.get_conflicting_bookings(
            tenant_id, data.start_time, data.end_time
        )
        if conflicts:
...
class ApproveBookingUseCase:
    ...
    async def execute(self, booking_id: UUID) -> Booking:
        ...
        if not tenant.calcom_api_key:
            raise HTTPException(
                status_code=400, detail="Tenant not connected to Cal.com"
            )
...
class CancelBookingUseCase:
    ...
    async def execute(
        self, booking_id: UUID, reason: str = "Host cancelled"
    ) -> Booking:
        booking = await self.booking_repo.get_by_id(booking_id)
        if not booking:
            raise HTTPException(status_code=404, detail="Booking not found")
        ...
        if (
            ...
            and booking.calcom_booking_uid
        ):
            # Cancel in Cal.com
            tenant = await self.tenant_repo.get_by_id(booking.tenant_id)
