...
class SendMessageUseCase:
    ...
    async def execute(
        self, tenant_id: UUID, conversation_id: UUID, text: str
    ) -> Message:
...
class ProcessIncomingMessageUseCase:
    ...
    async def execute(self, tenant_id: UUID, payload: Dict[str, Any]):
        ...
        conversation = await self.conversation_repo.get_by_customer(
            tenant_id, customer.id
        )
        if not conversation:
            conversation = await self.conversation_repo.create(
                {"tenant_id": tenant_id, "customer_id": customer.id, "status": "ACTIVE"}
            )
