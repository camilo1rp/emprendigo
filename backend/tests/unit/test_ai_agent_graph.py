...
@pytest.mark.asyncio
@patch(
    "backend.application.ai_agent.nodes.booking.get_services_list",
...
async def test_booking_node_parsing(
    mock_from_messages, mock_llm_factory, mock_get_services
):
    from backend.application.ai_agent.nodes.booking import (
        booking_node,
        BookingExtraction,
    )
    ...
    mock_extraction = BookingExtraction(
        service_name="Haircut", datetime_slot="Tomorrow at 10 AM", notes=""
    )
    ...
    state: AgentState = {
        "messages": [
            HumanMessage(content="I want to book a Haircut for tomorrow at 10 AM")
        ],
...
@patch(
    "backend.application.ai_agent.nodes.booking.get_tenant_payment_config",
    new_callable=AsyncMock,
)
@patch(
    "backend.application.ai_agent.nodes.booking.BookingRepository.create",
    new_callable=AsyncMock,
)
async def test_booking_node_confirmation(
    mock_create, mock_payment_config, mock_get_services
):
